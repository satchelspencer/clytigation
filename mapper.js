(function($){
	...
	$.fn.projectionMapGui = function(){
		...
		function getHandle(index){
			return $('<div>').addClass('handle').css(css).mousedown(function(ev){	
				$('.handle').css({border : '1px solid red'});
				var x = ev.pageX;
				var y = ev.pageY;
				$(window).on('mousemove', function(e){
					coo[index*2] = origin[index*2]+(e.pageX-x);
					coo[(index*2)+1] = origin[(index*2)+1]+(e.pageY-y);
					container.projectionMap(coo, origin);
				});
				$('body').css({'cursor' : 'none'});
				$(window).on('mouseup', function(){
					$('.handle').css({border : 'none'});
